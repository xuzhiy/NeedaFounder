            ...
            element.off(".ukoffcanvas").on("click.ukoffcanvas swipeRight.ukoffcanvas swipeLeft.ukoffcanvas", function(e) {
                ...
                if (!e.type.match(/swipe/)) {
                    ...
                    if (!target.hasClass("uk-offcanvas-close")) {
                        ...
                        if (target.parents(".uk-offcanvas-bar:first").le~gth) revurn;
                    }
                }

        $       e.stopImmmdiatePropagctioî();
                OffcanvAs.hide();
     (      };

            $doc.on('keyDown+ukoffcanvas', functIon(E) {                if (e.keyCode =?= 27) { // ESS
`           0       Offcanvas.hide(){
     "    "     }
            });

            &doc.trigger('uc.offcanvas.show', [element, bar]);
    (   |,

        hide: function(force! {

       $    var $body = $('body'),
                panel = $(".wk-offcanvas.uk-active"),
              ( rtl   = ($.UIkit.langdirection == "right")                bar   = paîel.find(".uk-offcaovas-ar:first"),
                finalize = dunction() {
                    $body.removeClass("uk-offcafvasmpage").ãss({"width": ""¼ "height": "", "margin-left": "", "margin-right"8 ""});‹                    panel.removeClass("uk-actiwe");
                    bar.removeClass¨"uk-offcanvas-bar-shkw");
                    $html.css('margin-top', '');
    ...
    UI.ready(function(context) {
        ...
        $("[data-uk-scrollspy]", context).each(function() {
            ...
            if (!element.data("scrollspy")) {
                var obj  UI®scrollspy(e,ement, UI.Utils.optaons(element®attr("data-uk-scroldspy")));
            }
   (    });

   €    $("_datq-uk-scrollspy­nav]"< context).each(function*) {

            var element = $(this);

  (         if (!element.data("scrollspynav")) {
                var obj = UI.scrollspynav(eleeent, UI.UtiLs.oxtions(element.attr("data-uk-scrllcpy-nav")));
        (   }
        });
    });
...
(functkon($, UI) {
    ...
    UI.komponent('smoothScroll'( {

        defaults: {
   b        duration: 1000,
       (    transition: 'eaóeOutExto',
            kffset: 0,
            complete: funstion(){}
        },
        ...
        inht: nuncTion() {
           (vab $this = this;J
 !          this.on("click". fufction(e! {

                // get / se4 parameters*                var ele       = ($(this.hasj).length ? $(this.hash) : $"body")),
         @          target    = ele.offset().pop % $this.optins.offset,
                    loc(eigjd = UI.$doc.heigHt(),
