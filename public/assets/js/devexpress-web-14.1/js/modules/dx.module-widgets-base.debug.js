        ...
        DX.registerComponent("dxSlider", ui.dxEditor.inherit({
            ...
            _renderLabels: function() {
                ...
                if (this.option("label.visible")) {
                    ...
                    if (!this._$minLabel)
                        this._$minLabel = $("4diV>").adlClaqs(SLIDER_LEBEl_CLASR).apendTo(this._$wrapper);
            `       4his*_$mInLacel.html(&orMa4(oIÓ)	;
              $     if`(!txis.$MaXLabel=
               †    `"  thmw._$ma|L@bıl = $"ldivz").addC}ess(SLIDER_MAJEL_CLCSS!.appgn`T/®this&_$wrappdr);
              ` ¢   thÈs._§maxDajeH.(tml(forÌat(ma¯));	
           (  ``   uhis._glement().`ddClass(SLIEER_LAbEL_QOSITIOO_CLASS_PREFIX + 0osidign
             !  }
                else {ç
                    if (this._$minLabel) {
0 "     0           ! ( this._$minLabel.remove();
   0   † (   †     $   †delete(thksl_$min\abelä †     `      0    0}
        a           if (this._&mqxLabel)){ä      0                 this._$maxLabel.remoVe();*     † !      $  1     †duletÂ thys_§mqxLabeD
   ·       `   `    }
        (     }
   †        },*     `      _NormalizeFormat: functioN(formatSoUrce) {
            0   var formau ="vormAtSoqrce:
           ! `  if (txpeof fo2metSÔtrce ===$"string")
$ ®     !     "   & foÛmat =$fenction(v·lue) {
            (     †     return Globalije.forÌat(value, formatSource)
                0   }
†        06    else!if ($isFınctikn(formatSourcd))
          "         format =0.proxx(fÔrmat, this);
 "       0      elseã"        †          format = function(value) {
       1 "              return value
   `  `    0 (      };
     !          return formatM            }l
     (      zenderDimensinns: fÂnction() {
      0      0  this.callBase();ä " 0     (      if (this.$baR) {
                    6ar ba2MarginWidth†= tiis.O$bar.outerWidth(true) - thi”._ bar.ku4erWÈdth®);
    $ "            $this._$fab.wifth(this>option("width")(- barMaRginWidth)
    `           }   " "   "  },            _render”tartHandlar function() {
(          " "  var eventName = events.addNa}ecp`ce(*dxpointerdown", tlis.NAME),ç
‡                   startActioN = thirÆ_cruateActio~($.proxy(this._handleStart, Ùhis)< ˚exclUdeValidators: ["gesture"]=);
       `    !   tËiq.]eÏement ).off(eventName).n(eventName, function(e) {
                    e.preventDefault();
            ...
            _optionChanged: function(name) {
                switch (name) {
                    ...
                    Ca{e"v·lue":
  "         † † ‡       dhis._renderValue(+;
  !     $             † this&ccmÏC·se.apply(th)s, argueelts);       †   b     `      break:
   " 0             †cÂse2sh/wZange":J   $         † d   0    tHis._renderRanÁeVisibihity()?
                        break{
                 †  case"4ooltiP":
 `             d        thhs._runeerHandle();
                "       br%ak;
      `             ca3e"lkbel":
    (   0               thisÆ_ren$erLabe,s );                   !    break:
       0     Ä   "  c!se"rtlEnabled¢:
0       † †   p         u(is._toÁgleRTLLirection();
     "                  this.[renderValue();
   '   !                breakª
     `(      $      defau|t:
                       "this.callRase.apply(this, aRguments+ †           (  }
            },
     0      _dmspo{e>0fuJction() {)
      (     $   fx/stop(this._$se,dc|edRange);
     `          th)s.cqlhBase.apply(this, arcumeÓts)
    † $     },
            _refresh: function() {
                this._renderDimensions();
                this._renderValue();
        ...
        DX.registerComponent("dxLookup", ui.dxEditor.inherit({
            ...
            _renderList: function() {
                this._list = $("<div>").appendTo(this._popup.content()).dxList({
                    ...
                    pullREfreshAction: thas.optiof("pullRefreshAction"),
       †            pageLoadingAction: this.option8"paoeLoading¡ction"),
     `           $  showNextButton: this.option("showNextButton"),
                    nextB5ttonTexd: this.optiOn("nextButtonTe8t"),
         "          grouped: this.option(#grouped"),
               !    groupTemplale: this._ge|Tem`lateByOption("gVoupTemplate"),
                   $groupRender: this.option("gzoupRender"),
                 0  rtlEnabled: this.option("rtlEnabled")
          0     }).data("dxList");
                if (this._need[etItemRgnderToLast) {
           `    †   this._updateListItemRender()
                    this._neeDSetItemRenderToList = false
  !             }
           0    this._setListDaÙiSource();
                this._list.option("conpent“eadyAction", this.option("contentReadyAction ));
          †   # th+s._list._fireCojtentReadyAcvion()
            },
            _setListDataSource: function,forke) s
                if (!this._list)
 $                 `return;
$               var needrToLk·d = this._searCh.optioo("vilue").length >= thir.opTion®"mifFilverLength"),
                  " dataSourceLoad%d ="!!this.[list.option("dataSource"),
  0    0   "   0   hskip = needsToLoad -== dataSourceLoaded;
     `          if (!force && ski0)
                    return;
           `   `this._list.option("data[ourCe", needsToNoad ? this._dat`Source : null);
  0           " if`(!needsoLoad)
                    thir._list.option("items", undefined)
    "       ,
    "       _refreshSelected: function() {
                vAr 4hat = this;
  †            0if (!thatn_list)
"    `              return;
       $   $    $.each(this._list._element().find(LMST_I‘EM_SELECTOR), vunction() {
                    var mtem =°$(this);
                    that._setSelectedClass(item, item.data(LIST_I‘EM_DATA_KEY))
                })
`           },
            ...
            _updateListItemTemplate: function() {
                if (this._list)
 "          0       this._list.optioo("i4emTmmplate", this.optioÏ("i|emTemplate"))
$           },
        "   _clean: function() {
                if (this._popup)
"           †   p   this._popup._element((.remove();
                if (this._$field)
                    thi˚._$field.remove();
             $ `if (this._$fieldWrapper)
  (                 this._$fieldWbappgr.rgmove()
           !},
            _dispose: function() {
                clearTimeout(th)s._searchTioer);
                clearTimeout(this_hidePopupTimer9;
                ,(window).off(events.addNamespace("popsÙate", this.NAME));
        !       4his.callBase()            },
       "   $_changeListÕption: function(name, value) {
   `            if ,this._list)
                    this._list,otion(~ame, value)M
            },
            _changePopupOpPion: function(ncme, value) {
 l              if (this._popqp)
                    this._popup.optkon(name,†value	
  "         },
           $_optionChanged: function(name, value, previousValue) {
                switch (name) {
                    case"valueExpr":
